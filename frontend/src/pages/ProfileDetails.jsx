...
function ProfileDetails() {
  ...
  const [mobileFromReg, setMobileFromReg] = useState(false)
    ...
    const fetchExisting = async () => {
      try {
        ...
        if (s) {
          ...
          if (s.schoolCollege) setSchoolCollege(s.schoolCollege)
          if (s.education) setEducation(s.education)
          if (s.skills) setSkills(s.skills)
          if (s.hobbies) setHobbies(s.hobbies)
          if (s.workExperience) setWorkExperience(s.workExperience)
          if (s.preferredWorkLocation) setPreferredWorkLocation(s.preferredWorkLocation)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/profile/complete`, {
        contactNumber, address, schoolCollege, education,
        skills, hobbies, workExperience, preferredWorkLocation,
        pfAccountNumber, photoUrl
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl p-7 shadow-sm">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={() => navigate('/dashboard')} className="text-gray-400 text-sm font-bold py-1">Skip for now</button>
          </div>
        </div>
      </div>
    </div>
